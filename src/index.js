...
const ICON_ARROW_RIGHT = <svg viewBox="0 0 13.409 8.091"><path id="arrow-down" d="M6.53,18.86l6.279,5.956L18.53,18.86" transform="translate(-5.842 -18.134)" fill="none" stroke="currentColor" strokeWidth="1.5"/></svg>;
const ICON_DUPLICATE = <svg viewBox="0 0 17.752 19.052"><g id="copy" transform="translate(0.875 0.875)"><rect width="13.001" height="14.124" rx="1.25" transform="translate(0 3.177)" fill="none" stroke="currentColor" strokeLinecap="square" strokeWidth="1.75"/><path d="M19.348,11.27v-.437a1.345,1.345,0,0,1,1.276-1.4h10.3a1.345,1.345,0,0,1,1.276,1.4V22.151a1.345,1.345,0,0,1-1.276,1.4h-.441" transform="translate(-16.197 -9.43)" fill="none" stroke="currentColor" strokeLinecap="square" strokeWidth="1.75"/></g></svg>;
const ICON_SAVE_TEMPLATE = <svg viewBox="0 0 15.717 17.443"><path id="attachment" d="M25.171,17.179l-6.447,6.44a4.474,4.474,0,0,1-6.074-.163C10.9,21.7,10.605,19.2,12.306,17.444c1.813-1.875,5.526-5.526,7.506-7.506a2.953,2.953,0,0,1,4.413.025,2.928,2.928,0,0,1,0,4.286L17.2,21.3a1.672,1.672,0,0,1-2.5-.022,1.722,1.722,0,0,1,.131-2.415l6.527-6.487" transform="translate(-10.198 -8.053)" fill="none" stroke="currentColor" strokeWidth="1.5"/></svg>;
...
class Card extends React.Component {
  ...
  render() {
    ...
    let rendered_tags = this.renderTags();
  ...
  renderTags() {
    ...
    try {
      ...
      while(large_tag_count * 3.5 + small_tag_count * 1.0 > column_width) {
        large_tag_count--;
        small_tag_count++;
      }
  ...
  componentDidMount() {
    console.log("    Card " + this.state.name + " mount");
    this.render();
  }
...
class DisplayedCard extends Card {
  ...
  render() {
    ...
    this.state.tags.forEach(tag => {
      try {
        tags.push(<span class={`tag-color-${window.board.tags[tag].color}`}>{window.board.tags[tag].name}</span>);
      }
      catch(e) {
        console.log(window.board.tags[tag]);
      }
    });
    tags.push(<span id="add-tag">{ICON_ADD_TAG}</span>);

    let assignees = []
    this.state.assignees.forEach(assignee => {
      assignees.push(
        <li class="displayed-assignee">
        ...
        <div className={"displayed-card theme-" + window.theme}>
          ...
          <div className="displayed-card-details">
            <div className="displayed-card-section displayed-card-tags">
              ...
              <div class="displayed-card-tag-list">{tags}</div>
            </div>
            ...
            <div className="displayed-card-options">
              <div id="move-card-button">
                {ICON_ARROW_RIGHT}<span>Move</span>
              </div>
              <div id="duplicate-card-button">
                {ICON_DUPLICATE}<span>Duplicate</span>
              </div>
              <div id="save-template-card-button">
...
var boardContainer = 
            <div>
              ...
              <DisplayedCard id={1} name="Call a function from its name/id in order to allow arrays of functions." description={"Add an option to call functions from its name/id for more flexibility, specifically to make lists of functions. Alternatively make some kind of function object."} column={1} tags={[0, 1, 4]} assignees={[1, 3]}/>
            </div>;
